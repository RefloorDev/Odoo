...
class ResCompany(models.Model):
	...
	recision_date = fields.Date('RecisionDate')
...
class Product(models.Model):
	...
	def _compute_labor_cost(self):
		for record in self:
			...
			labor_cost=self.env['labor.cost'].search(['|','&',('from_date','<=',date_today),('to_date','>=',date_today),'&',('from_date','<=',date_today),('to_date','=',False)])
			if labor_cost:
