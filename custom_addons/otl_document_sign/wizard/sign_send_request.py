...
class SignSendRequest(models.TransientModel):
    ...
    def default_get(self, fields):
        ...
        template = self.env['otl_document_sign.template'].browse(res['template_id'])
        res['filename'] = template.display_name
        # res['reference'] = template.model_id and template.model_id.model or False
    ...
    def send_request(self):
        ...
        request = self.env['otl_document_sign.request'].browse(res['id'])
...
class SignSendRequestSigner(models.TransientModel):
    _name = "otl_document_sign.send.request.signer"
    _description = 'Sign send request signer'

    role_id = fields.Many2one('otl_document_sign.item.role', readonly=True, required=True)
    partner_id = fields.Many2one('res.partner', required=True, string="Contact")
    sign_send_request_id = fields.Many2one('otl_document_sign.send.request')
    ...
    def create(self, vals_list):
        ...
        for vals in vals_list:
            if not vals.get('partner_id'):
                ...
                role = self.env['otl_document_sign.item.role'].browse(role_id)
