...
#modified
    def get_document_qweb_context(self, id, token):
        sign_request = http.request.env['otl_document_sign.request'].sudo().browse(id)
        if not sign_request:
            if token:
                return http.request.render('otl_document_sign.deleted_sign_request')
            else:
        ...
        sign_item_types = http.request.env['otl_document_sign.item.type'].sudo().with_context(active_test=False).search_read([])
        if current_request_item:
            for item_type in sign_item_types:

                if item_type['auto_field']:
                    fields = item_type['auto_field'].split('.')
                    selected_record =http.request.env[current_request_item.model_id.model].sudo().search([('id','=',current_request_item.res_id)],limit=1)
                    ...
                    if isinstance(auto_field, date):
                        lg = http.request.env['res.lang']._lang_get(http.request.env.user.lang)
        ...
        sr_values = http.request.env['otl_document_sign.request.item.value'].sudo().search([('sign_request_id', '=', sign_request.id)])
        item_values = {}
        for value in sr_values:
            item_values[value.sign_item_id.id] = value.value

        Log = request.env['otl_document_sign.log'].sudo()
    ...
    def sign_document_from_mail(self, id, token):
        sign_request = request.env['otl_document_sign.request'].sudo().browse(id)
        if not sign_request:
            return http.request.render('otl_document_sign.deleted_sign_request')
        current_request_item = sign_request.request_item_ids.filtered(lambda r: r.access_token == token)
    ...
    def sign_document_public(self, id, token, **post):
        ...
        return http.request.render('otl_document_sign.doc_sign', document_context)
    ...
    def download_document(self, id, token, download_type, **post):
        ...
        if download_type == "log":
            report_action = http.request.env.ref('otl_document_sign.action_sign_request_print_logs').sudo()
    ...
    def share_link(self, link, **post):
        template = http.request.env['otl_document_sign.template'].sudo().search([('share_link', '=', link)], limit=1)
        if not template:
            return http.request.not_found()

        sign_request = http.request.env['otl_document_sign.request'].with_user(template.create_uid).create({
            'template_id': template.id,
            'reference': "%(template_name)s-public" % {'template_name': template.attachment_id.name},
            'favorited_ids': [(4, template.create_uid.id)],
        })

        request_item = http.request.env['otl_document_sign.request.item'].sudo().create({'sign_request_id': sign_request.id, 'role_id': template.sign_item_ids.mapped('responsible_id').id})
    ...
    def check_password_page(self, sign_request_id, token, **post):
        ...
        request_item = http.request.env['otl_document_sign.request.item'].sudo().search([
        ...
        if 'password' not in http.request.params:
            return http.request.render('otl_document_sign.encrypted_ask_password')
    ...
    def get_document(self, id, token):
        return http.Response(template='otl_document_sign._doc_sign', qcontext=self.get_document_qweb_context(id, token)).render()
    ...
    def sign_get_user_signature(self, request_id, item_access_token, signature_type='signature'):
        sign_request_item = http.request.env['otl_document_sign.request.item'].sudo().search([
    ...
    def make_public_user(self, id, token, name=None, mail=None):
        sign_request = http.request.env['otl_document_sign.request'].sudo().search([('id', '=', id), ('access_token', '=', token)])
    ...
    def send_sms(self, id, token, phone_number):
        request_item = http.request.env['otl_document_sign.request.item'].sudo().search([('sign_request_id', '=', id), ('access_token', '=', token), ('state', '=', 'sent')], limit=1)
        if not request_item:
            return False
    ...
    def sign(self, id, token, sms_token=False, signature=None):
        ...
        sign_users = user_ids.filtered(lambda u: u.has_group('otl_document_sign.group_sign_user'))
    ...
    def check_encrypted(self, sign_request_id):
        request_item = http.request.env['otl_document_sign.request.item'].sudo().search([('sign_request_id', '=', sign_request_id)], limit=1)
        if not request_item:
            return False
    ...
    def save_location(self, id, token, latitude=0, longitude=0):
        sign_request_item = http.request.env['otl_document_sign.request.item'].sudo().search([('sign_request_id', '=', id), ('access_token', '=', token)], limit=1)
