odoo.define('otl_document_sign.PDFIframe', function (require) {
    'use strict';
    ...
    var PDFIframe = Widget.extend({
        ...
        doPDFPostLoad: function() {
            ...
            var $cssLink = $("<link/>", {
                ...
                href: "/otl_document_sign/static/src/css/iframe.css"
            });
            var $faLink = $("<link/>", {
                rel: "stylesheet", type: "text/css",
        ...
        createSignItem: function (type, required, responsible, posX, posY, width, height, value, option_ids, name,tip,option_field) {
            var readonly = this.readonlyFields || (responsible > 0 && responsible !== this.role) || !!value;
            var selected_options = option_ids || [];

            var $signatureItem = $(core.qweb.render('otl_document_sign.sign_item', {
                ...
                placeholder: '',
                option_field: (name) ? name : type['option_field']
            }));
            ...
            return $signatureItem.data({type: type['id'], required: required, responsible: responsible, posx: posX, posy: posY, width: width, height: height, name:name, option_ids: option_ids,tip:tip,option_field:option_field})
                                 .data('hasValue', !!value);
        },
...
odoo.define('otl_document_sign.Document', function (require) {
    'use strict';

    var ajax = require('web.ajax');
    var core = require('web.core');
    var Dialog = require('web.Dialog');
    var PDFIframe = require('otl_document_sign.PDFIframe');
    var Widget = require('web.Widget');
...
odoo.define('otl_document_sign.utils', function (require) {
    'use strict';
    ...
    function getOptionsSelectConfiguration(item_id, select_options, selected) {
        if(getOptionsSelectConfiguration.configuration === undefined) {
            ...
            var selectChangeHandler = function(e) {
                ...
                ajax.rpc('/web/dataset/call_kw/otl_document_sign.template/add_option', {
                    model: 'otl_document_sign.template',
                    method: 'add_option',
                    args: [value],
                    kwargs: {}
                }).then(process_option);
    ...
    function getResponsibleSelectConfiguration(parties) {
        if(getResponsibleSelectConfiguration.configuration === undefined) {
            ...
            var selectChangeHandler = function(e) {
                ...
                ajax.rpc('/web/dataset/call_kw/otl_document_sign.item.role/add', {
                    model: 'otl_document_sign.item.role',
                    method: 'add',
                    args: [name],
                    kwargs: {}
                }).then(process_party);
...
// Signing part
odoo.define('otl_document_sign.document_signing', function (require) {
    'use strict';

    var ajax = require('web.ajax');
    var core = require('web.core');
    var Dialog = require('web.Dialog');
    var Document = require('otl_document_sign.Document');
...
//        template: 'otl_document_sign.signature_dialog',
//
//        custom_events: {
...
//    });
    ...
    var SignItemNavigator = Widget.extend({
        ...
        start: function() {
            ...
            var i;
           for (i = 0; i < _.keys(this.types).length; i++) {
               this.$el[0].click();
             }
...
//                self.getParent().refreshSignItems();
    ...
    var PublicSignerDialog = Dialog.extend({
        template: "otl_document_sign.public_signer_dialog",
    ...
    var SMSSignerDialog = Dialog.extend({
        template: "otl_document_sign.public_sms_signer",
    ...
    var EncryptedDialog = Dialog.extend({
        template: "otl_document_sign.public_password",
    ...
    var ThankYouDialog = Dialog.extend({
        template: "otl_document_sign.thank_you_dialog",
        events: {
        ...
        init: function(parent, RedirectURL, RedirectURLText, requestID, options) {
            ...
            options.subtitle = options.subtitle || _t("The order and contract have been submitted.");
            options.size = options.size || "medium";
            options.technical = false;
            options.buttons = [];
            if (RedirectURL) {
            ...
            if (! session.uid) {
                ...
                window.location.reload();
                    ...
                    var NextAction = function() {
                        return self.do_action('otl_document_sign.sign_template_action', {
...
//            this.$modal.find('button.close').addClass('invisible');
    ...
    var NextDirectSignDialog = Dialog.extend({
        template: "otl_document_sign.next_direct_sign_dialog",
        events: {
            'click .o_go_to_document': 'on_closed',
        ...
        init: function(parent, RedirectURL, requestID, options) {
            ...
            options.subtitle = options.subtitle || _t("The order and contract have been submitted.") + " " +_.str.sprintf(_t("Next signatory is %s"), this.name_list[0]);
            options.size = options.size || "medium";
            options.technical = false;
        ...
        on_click_next: function () {
            ...
            self.do_action({
                ...
                tag: 'otl_document_sign.SignableDocument',
                name: _t("Sign"),
            }, {
                additional_context: {
                    id: this.requestID,
    ...
    var SignablePDFIframe = PDFIframe.extend({
        ...
        createSignItem: function(type, required, responsible, posX, posY, width, height, value, options, name,tip,option_field) {
            var self = this;
            ...
            if(!readonly) {
                ...
                all_fields[$signatureItem.data('type')] = type['option_field'];
                if (type['item_type'] === "signature" || type['item_type'] === "initial") {
                    $signatureItem.on('click', function(e) {

                        self.refreshSignItems();

//                        $validateBanner = self.$('.o_sign_validate_banner').first();
//                        $validateBanner.hide().css('opacity', 0);
                        var image='data:image/png;base64,'+ type.auto_field

                        if ($signatureItem) {
                            if (type['tip'] === "filled in") {
                                $signatureItem.removeData( 'signature').empty()
                                                 .append($('<span/>').addClass("o_sign_helper"), type['placeholder']);
                                type['tip'] = "fill in"
//                              type['autofield'] = ''
                                $signatureItem.val("");
                                if (type['option_field']) {

                                                            option_dict[type['option_field']] = undefined;
                                ...
                                $signatureItem.data('signature', image)
                                               .empty()
                                                 .append($('<span/>').addClass("o_sign_helper"), $('<img/>', {src: $signatureItem.data('signature')}));
                                type['tip'] = "filled in"
                                        if (type['option_field']) {

                                        option_dict[type['option_field']] = $signatureItem.data('type');
                        ...
                        var $signedItems = self.$('.o_sign_sign_item').filter(function(i) {
                            var $item = $(this);
                            if (type['option_field'] && all_fields[$item.data('type')] == type['option_field'])
                                {
                                        if($item.data('type') != option_dict[type['option_field']] )
                                        {
                                        $item.removeData( 'signature').empty()
...
//                            });
                    ...
                    $signatureItem.prop('readonly', true);
                    if(type.item_type === 'checkbox') {
                        $signatureItem.attr("onclick", "return false;");
                    }
                }
                if(type['auto_field']) {
                    ...
                    if(type.item_type === 'checkbox') {
                        if (type.auto_field ==="True")
                            {
                                $signatureItem.val("on");
                                $signatureItem.prop("checked", true);

                            }
                        else
                            {
                            $signatureItem.val("off");
                            $signatureItem.prop("checked", false);
                            }
...
//                if ($(el)[0].type == 'submit')
//                {
//                    var image='data:image/png;base64,'+$elem.val()
//                    $elem.data('signature', image).empty().append($('<span/>').addClass("o_sign_helper"), $('<img/>', {src: $elem.data('signature')}));
//                }
//                if ($(el)[0].type == 'checkbox' && $elem.val() == 'on')
//                    {
//                       $(el).prop("checked", true);
//                    }
                 if ($(el)[0].type == 'checkbox' && $elem.val() == 'off')
                    {
                       $(el).prop("checked", false);
                    }
                var unchecked_box = $elem.val() == 'on' && !$elem.is(":checked")
                return !(($elem.val() && $elem.val().trim()) ) || unchecked_box;
            });
    ...
    var SignableDocument = Document.extend({
        events: {
            'pdfToComplete .o_sign_pdf_iframe': function(e) {
                this.$validateBanner.show().animate({'opacity': 400}, 500);
            ...
            for(var page in this.iframeWidget.configuration) {
                    ...
                    for(var i = 0 ; i < this.iframeWidget.configuration[page].length ; i++) {
                        var $elem = this.iframeWidget.configuration[page][i];
                         if ($elem[0].type ==='submit')
                        {

                        for(var key  in option_dict){

                            if (option_dict[key] === undefined)
                                {
                                Dialog.alert(this, _t("Please Click to Initial Each Section and Sign at the Bottom."), {title: _t("Warning")});
                                return;
                                }

                            }

                     if($elem.data('required') && $elem[0].value=="")
                        {
