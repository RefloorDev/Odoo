odoo.define('otl_document_sign.views_custo', function(require) {
    'use strict';
...
//    ListController.include(_make_custo(".o_list_button_add"));
    ...
    KanbanColumn.include({
        ...
        init: function () {
            ...
            if (this.modelName === "otl_document_sign.request") {
    ...
    KanbanRecord.include({
        ...
        _openRecord: function () {
            var self = this;
            if (this.modelName === 'otl_document_sign.template' && this.$el.parents('.o_kanban_dashboard').length) {
                self._rpc({
            ...
            } else if (this.modelName === 'otl_document_sign.request' && this.$el.parents('.o_sign_request_kanban').length) {
                this._rpc({
                    model: 'otl_document_sign.request',
    ...
    function _make_custo(selector_button) {
        return {
            renderButtons: function () {
                ...
                if (this.modelName === "otl_document_sign.template") {
                    this._sign_upload_file_button();
                    this.$buttons.find('button.o_button_import').hide();
                } else if (this.modelName === "otl_document_sign.request") {
                    if (this.$buttons) {
    ...
    function _sign_create_request() {
        this.do_action("otl_document_sign.sign_template_action");
    }
});

odoo.define('otl_document_sign.template', function(require) {
    'use strict';
    ...
    var sign_utils = require('otl_document_sign.utils');

    var _t = core._t;

    var SignItemCustomPopover = Widget.extend({
        template: 'otl_document_sign.sign_item_custom_popover',
        events: {
    ...
    var InitialAllPagesDialog = Dialog.extend({
        template: 'otl_document_sign.initial_all_pages_dialog',
            ...
            this.fullyLoaded.then(function() {
                if(self.editMode) {
                    if(self.$iframe.prop('disabled')) {
                        ...
                        var $fieldTypeButtons = $(core.qweb.render('otl_document_sign.type_buttons', {sign_item_types: typesArr}));
    ...
    var Template = AbstractAction.extend({
        ...
        go_back_to_kanban: function() {
            return this.do_action("otl_document_sign.sign_template_action", {
        ...
        init: function(parent, options) {
            ...
            var $sendButton = $('<button/>', {html: _t("Send"), type: "button"})
                .addClass('btn btn-secondary')
                .on('click', function () {
                    self.do_action('otl_document_sign.action_sign_send_request', {
                        additional_context: {
            ...
            var $shareButton = $('<button/>', {html: _t("Share"), type: "button"})
                .addClass('btn btn-secondary')
                .on('click', function() {
                    self.do_action('otl_document_sign.action_sign_template_share', {
                        additional_context: {
                            active_id: self.templateID,
                        },
                    });
        ...
        perform_rpc: async function() {
                ...
                .then(function prepare_template(template) {
                    ...
                    var defSignItems = self._rpc({
                        ...
                        .then(function (sign_items) {
                            ...
                            return self._rpc({
                                model: 'otl_document_sign.item.option',
                                method: 'search_read',
                                args: [[],['id', 'value']],
                                kwargs: {context: session.user_context},
                            });
            ...
            var defItemTypes = await this._rpc({
                    model: 'otl_document_sign.template',
                    method: 'read',
                    args: [[this.templateID]],
                })
                .then(async function prepare_template(template) {
                    template = template[0];
                    var template_model_id = template['model_id'][0]
                  await  self._rpc({
                            model: 'otl_document_sign.item.type',
                            method: 'search_read',
                            args: [['|', ['model_id', '=',template_model_id], ['model_id', '=', false]]],
                            kwargs: {context: session.user_context},
        ...
        initialize_content: function() {
            this.$('.o_content').append(core.qweb.render('otl_document_sign.template', {widget: this}));
            ...
            for(var page in configuration) {
                for(var i = 0 ; i < configuration[page].length ; i++) {
                    ...
                    data[id] = {
                        ...
                        'tip' : configuration[page][i].data('tip'),
                        'option_field': configuration[page][i].data('option_field')
                    };
    ...
    core.action_registry.add('otl_document_sign.Template', Template);
});

odoo.define('otl_document_sign.DocumentBackend', function (require) {
    'use strict';
    var AbstractAction = require('web.AbstractAction');
    var core = require('web.core');
    ...
    var DocumentBackend = AbstractAction.extend({
        ...
        go_back_to_kanban: function () {
            return this.do_action("otl_document_sign.sign_request_action", {
                clear_breadcrumbs: true,
            });
        },
...
odoo.define('otl_document_sign.document_edition', function(require) {
    'use strict';

    var core = require('web.core');
    var session = require('web.session');
    var DocumentBackend = require('otl_document_sign.DocumentBackend');
    ...
    var EditableDocumentBackend = DocumentBackend.extend({
        events: {
            'click .o_sign_resend_access_button': function(e) {
                ...
                this._rpc({
                        model: 'otl_document_sign.request.item',
    ...
    core.action_registry.add('otl_document_sign.Document', EditableDocumentBackend);
});

odoo.define('otl_document_sign.document_signing_backend', function(require) {
    'use strict';

    var core = require('web.core');
    var DocumentBackend = require('otl_document_sign.DocumentBackend');
    var document_signing = require('otl_document_sign.document_signing');

    var _t = core._t;

    var NoPubThankYouDialog = document_signing.ThankYouDialog.extend({
        template: "otl_document_sign.no_pub_thank_you_dialog",
    ...
    core.action_registry.add('otl_document_sign.SignableDocument', SignableDocumentBackend);
});
