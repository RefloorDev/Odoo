...
class SignLog(models.Model):
    _name = 'otl_document_sign.log'
    _order = 'log_date, id'
    _description = "Sign requests access history"

    # Accessed on ?
    log_date = fields.Datetime(default=fields.Datetime.now, required=True)
    sign_request_id = fields.Many2one('otl_document_sign.request', required=True)
    sign_request_item_id = fields.Many2one('otl_document_sign.request.item')
    # Accessed as ?
    user_id = fields.Many2one('res.users', groups="otl_document_sign.group_sign_manager")
    ...
    request_state = fields.Selection([
        ...
        ("signed", "Fully Signed"),
        ("canceled", "Canceled")
    ], required=True, string="State of the request on action log", groups="otl_document_sign.group_sign_manager")
    ...
    def unlink(self):
        for record in self:
            if not record._context.get('delete_log',False) is True:
                raise ValidationError(_("Log history of sign requests cannot be deleted !"))
        return super(SignLog, self).unlink()
    ...
    def _get_or_check_hash(self, vals):
        ...
        if not prev_activity:
            sign_request = self.env['otl_document_sign.request'].browse(vals['sign_request_id'])
    ...
    def _compute_string_to_hash(self, vals):
        ...
        item_values = self.env['otl_document_sign.request.item.value'].search(
            [('sign_request_id', '=', vals['sign_request_id'])]).filtered(
            lambda item: item.sign_request_item_id.access_token == vals['token'])
