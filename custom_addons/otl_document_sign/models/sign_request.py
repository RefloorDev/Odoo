...
class SignRequest(models.Model):
    _name = "otl_document_sign.request"
    ...
    request_item_ids = fields.One2many('otl_document_sign.request.item', 'sign_request_id', string="Signers")
    ...
    sign_log_ids = fields.One2many('otl_document_sign.log', 'sign_request_id', string="Logs", help="Activity logs linked to this request")
    ...
    def go_to_document(self):
        ...
        return {
            ...
            'tag': 'otl_document_sign.Document',
            'context': {
                'id': self.id,
    ...
    def open_request(self):
        ...
        return {
            ...
            "res_model": "otl_document_sign.request",
            "views": [[False, "form"]],
            "res_id": self.id,
        }
    ...
    def action_sent_without_mail(self):
        ...
        for sign_request in self:
            for sign_request_item in sign_request.request_item_ids:
                ...
                Log = self.env['otl_document_sign.log'].sudo()
    ...
    def action_sent(self, subject=None, message=None):
        ...
        for sign_request in self:
            ...
            if sign_request.send_signature_accesses(subject, message, ignored_partners=ignored_partners):
                ...
                vals['action'] = 'create'
                vals = Log._update_vals_with_http_request(vals)
                if 'ip'in vals  and 'user_id' in vals:
                    Log.create(vals)
...
#modified
    def set_signers(self, signers):
        SignRequestItem = self.env['otl_document_sign.request.item']
        for rec in self:
    ...
    def send_follower_accesses(self, followers, subject=None, message=None):
        ...
        tpl = self.env.ref('otl_document_sign.sign_template_mail_follower')
        body = tpl.render({
            'record': self,
    ...
    def send_completed_document(self):
        ...
        report_action = self.env.ref('otl_document_sign.action_sign_request_print_logs')
    ...
    def generate_completed_document_credit_card_application(self, password=""):
        ...
        for p in range(0, old_pdf.getNumPages()):
            ...
            for item in items:
                ...
                if item.type_id.item_type == "text":
                    can.setFont(font, height*.009*0.8)
                    can.drawString(width*item.posX, height*(1-item.posY-.009*0.9), value)
                    ...
                    print(item.type_id.name)
                    print(value)
                    can.setFont(font, height*0.012*0.8)
                    value = '✔' if value == 'True' else ''
                    print(value)
                    can.drawString(width*item.posX, height*(1-item.posY-0.012*0.9), value)

                elif item.type_id.item_type == "signature" or item.type_id.item_type == "initial":
                    if value != '0.0':
                        image_reader = ImageReader(io.BytesIO(base64.b64decode(value[value.find(',') + 1:])))
                        _fix_image_transparency(image_reader._image)
                        can.drawImage(image_reader, width * item.posX, height * (1 - item.posY - item.height), 70, 40,'auto', True)
    ...
    @api.model
    def initialize_new(self, record_id, template_id, signers, followers, reference, subject, message, send=True, without_mail=False):
        #modified
        sign_users = self.env['res.users'].search([('partner_id', 'in', [signer['partner_id'] for signer in signers])]).filtered(lambda u: u.has_group('otl_document_sign.group_sign_user'))
        sign_request = self.create(
            {'template_id': template_id,
             'res_id': record_id,
             'reference': reference,
             'favorited_ids': [(4, item) for item in (sign_users | self.env.user).ids]
             })
...
class SignRequestItem(models.Model):
    _name = "otl_document_sign.request.item"
    ...
    sign_request_id = fields.Many2one('otl_document_sign.request', string="Signature Request", ondelete='cascade', required=True)
    sign_item_value_ids = fields.One2many('otl_document_sign.request.item.value', 'sign_request_item_id', string="Value")

    access_token = fields.Char('Security Token', required=True, default=_default_access_token, readonly=True)
    access_via_link = fields.Boolean('Accessed Through Token')
    role_id = fields.Many2one('otl_document_sign.item.role', string="Role")
...
#modified
    model_id = fields.Many2one('ir.model', string="Model", related='sign_request_id.model_id')
    res_id = fields.Integer(string="Selected Resource ID", related='sign_request_id.res_id')
    ...
    def action_draft(self):
        ...
        for request_item in self:
            ...
            self.env['otl_document_sign.request.item.value'].search([('sign_item_id', 'in', itemsToClean.mapped('id')), ('sign_request_id', '=', request_item.sign_request_id.id)]).unlink()
...
class SignRequestItemValue(models.Model):
    _name = "otl_document_sign.request.item.value"
    _description = "Signature Item Value"
    _rec_name = 'sign_request_id'

    sign_request_item_id = fields.Many2one('otl_document_sign.request.item', string="Signature Request item", required=True,
                                           ondelete='cascade')
    sign_item_id = fields.Many2one('otl_document_sign.item', string="Signature Item", required=True, ondelete='cascade')
    ...
    value = fields.Text()
    type_id = fields.Many2one('otl_document_sign.item.type', string="Field Type", related='sign_item_id.type_id')
