...
class SignTemplate(models.Model):
    _name = "otl_document_sign.template"
    ...
    sign_item_ids = fields.One2many('otl_document_sign.item', 'template_id', string="Signature Items", copy=True)
    ...
    sign_request_ids = fields.One2many('otl_document_sign.request', 'template_id', string="Signature Requests")

    tag_ids = fields.Many2many('otl_document_sign.template.tag', string='Tags')
    ...
    def update_from_pdfviewer(self, template_id=None, duplicate=None, sign_items=None, name=None):
        ...
        for item in sign_items.values():
            ...
            self.env['otl_document_sign.item'].create(item)
    ...
    def add_option(self, value):
        option = self.env['otl_document_sign.item.option'].search([('value', '=', value)])
        option_id = option if option else self.env['otl_document_sign.item.option'].create({'value': value})
        return option_id.id
...
class SignTemplateTag(models.Model):
    ...
    _name = "otl_document_sign.template.tag"
...
class SignItemSelectionOption(models.Model):
    _name = "otl_document_sign.item.option"
...
class SignItem(models.Model):
    _name = "otl_document_sign.item"
    _description = "Fields to be sign on Document"
    _rec_name = 'name'

    template_id = fields.Many2one('otl_document_sign.template', string="Document Template", required=True, ondelete='cascade')

    type_id = fields.Many2one('otl_document_sign.item.type', string="Type", required=True, ondelete='cascade')

    required = fields.Boolean(default=True)
    responsible_id = fields.Many2one("otl_document_sign.item.role", string="Responsible")

    option_ids = fields.Many2many("otl_document_sign.item.option", string="Selection options")
...
class SignItemType(models.Model):
    _name = "otl_document_sign.item.type"
    ...
    option_field = fields.Char(string="Option Field")
    ...
    auto_field = fields.Char(string="Related Field", help="Model field to use to auto-complete the fields of this type")
    #modified
    model_id = fields.Many2one('ir.model', string='Model')
...
class SignItemParty(models.Model):
    _name = "otl_document_sign.item.role"
